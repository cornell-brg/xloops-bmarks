...
//========================================================================
// Tloops implementation of MiBench ADPCM decoder kernel. Modified all
// control-flow using predication support in form of CMOVE instruction.
// This optimization is applied to take advantage of the register value
// communication support in parallel lane execution of for.r, XLOOPS code.
// It results in a instruction bloat of 4 static instructions compared to
// the scalar kernel but should improve performance. Also, note that this
// implementation does useless work due computations which are predicated.
...
namespace mibench_adpcmdec {
                                 ...
                                 int len, adpcm_state* state)
  {
    ...
    // Decode input
    for ( int i = 0; i < len ; i++, ordered_for() )
    {
      ...
      //------------------------------------------------------------------
	    ...
	    if ( bufferstep ) {
	      ...
	      inputbuffer = ( indata[i/2] >> 4 );
	    }

      __asm__ __volatile__
      (
          "xori %[bufferstep], %[bufferstep], 1"
        : [bufferstep] "+r" ( bufferstep )
        :
      );
