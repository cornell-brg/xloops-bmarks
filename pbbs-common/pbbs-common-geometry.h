...
// ------------------------------------------------------------
// Custom inlineable square root implementation
// ------------------------------------------------------------
// Quake 3 Fast Square Root Approximation
// 
// The integer-shift approximation produced a relative error of
// less than 4%, and the error drops further to 0.15% with one
// iteration of Newton's method on the following line.
//
// Note: This must be a floating point due to the IEEE method for
// approx. of sqrt.
//
// Source: USC iLab Neuromorphic Vision C++ Toolkit
// (http://ilab.usc.edu/toolkit/)
// Distributed under the GNU General Public License
//

inline float fast_sqrt_Q3(const float x);
...
public: 
  ...
  floatT maxDim() {return std::max(x,std::max(y,z));}
  void print() {std::cout << std::setprecision(10) << ":(" << x << "," << y << "," << z << "):";}
  floatT Length(void) { return fast_sqrt_Q3(x*x+y*y+z*z);}
...
public: 
  ...
  floatT maxDim() {return std::max(x,y);}
  void print() {std::cout << ":(" << x << "," << y << "):";}
  floatT Length(void) { return fast_sqrt_Q3(x*x+y*y);}
  static const int dim = 3;
};
...
public: 
  ...
  floatT operator[] (int i) {return (i==0) ? x : y;};
  pointT minCoords(pointT b) { return pointT(std::min(x,b.x),std::min(y,b.y)); }
  pointT maxCoords(pointT b) { return pointT(std::max(x,b.x),std::max(y,b.y)); }
  int quadrant(pointT center) {
    int index = 0;
    if (x > center.x) index += 1;
    if (y > center.y) index += 2;
...
inline float triArea(point2d a, point2d b, point2d c) {
  return (b-a).cross(c-a);
}

inline float triAreaNormalized(point2d a, point2d b, point2d c) {
...
template <class floatT>
inline float inCircleNormalized(_point2d<floatT> a, _point2d<floatT> b, 
				 _point2d<floatT> c, _point2d<floatT> d) {
  _vect3d<floatT> ad = onParabola(a-d);
  _vect3d<floatT> bd = onParabola(b-d);
  _vect3d<floatT> cd = onParabola(c-d);
...
struct triangles {
  ...
  triangles(intT np, intT nt, point* __P, triangle* _T) 
    : numPoints(np), numTriangles(nt), P(__P), T(_T) {}
};
...
inline float angle(point2d a, point2d b, point2d c) {
  vect2d ba = (b-a);
  vect2d ca = (c-a);
  float lba = ba.Length();
  float lca = ca.Length();
  float pi = 3.14159;
  return 180 / pi * std::acos( ba.dot(ca) / (lba*lca) );
}

inline float minAngleCheck(point2d a, point2d b, point2d c, float angle) {
  vect2d ba = (b-a);
  vect2d ca = (c-a);
  vect2d cb = (c-b);
  float lba = ba.Length();
