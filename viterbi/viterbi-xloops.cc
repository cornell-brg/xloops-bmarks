...
namespace viterbi {
  ...
  int generate_symbol_bit_xloops(int x) {
    x ^= (x >> 16);
    x ^= (x >> 8);
    x ^= (x >> 4);
    x &= 0xf;
  ...
  void viterbi_xloops(unsigned char symbols[], unsigned char msg[],
    ...
    for (int state = 0; state < STATES/2; state++, unordered_for() ) {
      for (int i = 0; i < rate; i ++) {
        int bit = generate_symbol_bit_xloops(2*state & poly[i]);
    ...
    for (int s = 0; s < framebits + (K-1); s++) {
      for (int i = 0; i < STATES/2; i++, unordered_for()) {
